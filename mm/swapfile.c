...
unsigned int nr_swapfiles;//可用的交换页面数
...
void __swap_free(swp_entry_t entry, unsigned short count)
{
	...
	type = SWP_TYPE(entry);//返回的实际上是页面交换设备的序号，
	                                       //即其在swap_info 中的下标，swap_info
	                                       //最大大小为MAX_SWAPFILES = 8
	if (type >= nr_swapfiles)//页面不存在，也就是type 大于swap_info的大小
		goto bad_nofile;//读者可能会从nr_swapfiles的名字推断出
		//它就是交换文件（或者设备）的数目，
		//但是实际它不是。它是曾被使用过的swap_info的
		//最大索引值，而且从不会被降低。
		//（它记录着这个数组被使用的最高峰值。）
	p = & swap_info[type];//取得实际的页面交换swap_info_struct
	if (!(p->flags & SWP_USED))//表示该页面交换设备并未使用
		goto bad_device;
	offset = SWP_OFFSET(entry);//offset是页面在	交换设备中的位置，
	                               //也就是在文件中的逻辑页面号
	if (offset >= p->max)
	                               //不能大于其最大值
		goto bad_offset;
	if (!p->swap_map[offset])//p->swap_map[offset]是该页面的分配和
	                    //使用计数，如为0表示尚未分配
		goto bad_free;
	swap_list_lock();
	if (p->prio > swap_info[swap_list.next].prio)//swap_free函数检查是否
	//正在释放交换页面的设备具有比
	//随后将被考虑的设备更高的优先级。
	//如果是，它就把此作为一个线索以
	//将swap_list的迭代器重新设置在列表头部。
	//这样对get_swap_page函数的下一次调用
	//就会从列表头部开始并能够检测到
	//新近被释放的高优先级空间。
		swap_list.next = type;
	swap_device_lock(p);//swap_map对交换空间里每个交换页面的
	//用户数进行计数的数组；为0则表示页面空闲
	if (p->swap_map[offset] < SWAP_MAP_MAX) {//offset是页面在文件中的
	//位置，不能大于文件本身所提供的最大值?
	//应该是对swap_map每一页面的使用计数只维护到一
	//个最大值,即SWAP_MAP_MAX
		if (p->swap_map[offset] < count)//p->swap_map[offset] 是该页面的
		                                                //分配(和使用)计数
			goto bad_count;
		if (!(p->swap_map[offset] -= count)) {//表示没有用户要使用它了
			if (offset < p->lowest_bit)//跟踪交换设备里第一个和
		            //最后一个可用的页面的位置。这可以有助于
			    //加快寻找空闲页面的循环。设备的
			   //第一个页面是一个不允许用于交换的头部，
			   //因此lowest_bit不会是0。
			...
			nr_swap_pages++;//nr_swap_pages是交换设备的可用页面数
		}
	}
	swap_device_unlock(p);
	swap_list_unlock();
