 ...
 */
typedef struct page {
	struct list_head list;//页面拥有对象链表
	struct address_space *mapping;//页所在的文件空间the inode we belong to
	unsigned long index;//页所在文件空间的编号our offset within maping
	struct page *next_hash;//页缓冲散列链
	atomic_t count;//引用计数
	unsigned long flags;	/* atomic flags, some possibly updated asynchronously */
	struct list_head lru;
	unsigned long age;
	wait_queue_head_t wait;//	wait：是等待该页资源的进程等待队列的指针
