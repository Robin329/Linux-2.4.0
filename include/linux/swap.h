...
/*交换空间由若干个块设备文件物理空间或普通文件的物理空间构成,
每一交换文件用交换结构(swap_info_struct)描述, 最多允许8个交换文件(MAX_SWAPFILES),
交换文件按其优先级(swap_info->prio)大小顺序排列在交换链表(swap_list->head)中,
当前使用的交换文件为(swap_list->next). 每一交换结构具有页分配表(swap_info->swap_map).
*/
struct swap_info_struct {
	unsigned int flags;
	kdev_t swap_device;//Objects of type kdev_t designate a device 
	//交换设备, 当普通文件用作交换文件时, 其值为零
	spinlock_t sdev_lock;
	struct dentry * swap_file;// 交换文件所在的目录项
	struct vfsmount *swap_vfsmnt;//交换文件所在的文件系统
	unsigned short * swap_map;//有3种解释
	//(1)交换页分配表?
	//(2)对交换空间里每个交换页面的用户数进行计数的数组；
	//为0则表示页面空闲。 (根据程序来看，这个观点正确)
	//(3)指向一个数组，
	//该数组中的每一个无符号数即代表盘上的一个物理页面

	unsigned int lowest_bit;     
	unsigned int highest_bit;//lowest_bit和highest_bit——跟踪交换设备里
	//第一个和最后一个可用的页面的位置
	unsigned int cluster_next;//reference to your book on page 68
	unsigned int cluster_nr;
	int prio;			/* swap priority */
	int pages;//交换文件有效页面数
	unsigned long max;//页面分配表尺寸
 ...
 */
#define DEBUG_ADD_PAGE \//if  page is active ,inactive and dirty ,or inactive and clean ,then BUG() 
